...
class WordCounterTest {
	...
	void testWordCounter() {
        ...
        int authorCount = counters.get("Tolstoy");
        assertTrue(authorCount < 10 & authorCount >=1);

        assertEquals((int) counters.getOrDefault("LOUISE", 0), 0);
        assertEquals(1, (int) counters.get("Aylmer"));
	}
	...
	void testGetWordsOccuringMoreThan() {
		...
		ArrayList<String> ls4 = wc.getWordsOccuringMoreThan(-1);
        assertTrue(ls4.size() >= 41400 & ls4.size() <= 42400);

        ArrayList<String> ls5 = wc.getWordsOccuringMoreThan(20000);
        assertEquals(ls5.size(), 2);

        ArrayList<String> ls6 = wc.getWordsOccuringMoreThan(500);
        assertTrue(ls6.size() > 50);
	}
	...
	void testGenerateWordCounts() {
        ...
        expectedCounts.clear(); // clear all elements from the HashMap
        expectedCounts.put("war", 2);
		expectedCounts.put("and", 1);
		expectedCounts.put("the", 2);
		expectedCounts.put("peace", 2);
        expectedCounts.put("War", 1);
        expectedCounts.put("The", 2);

        wc = new WordCounter(lines);

        assertEquals(expectedCounts, wc.getWordCounter());
        assertTrue((int) wc.getWordCounter().get("war") >= 1);
        assertEquals((int) wc.getWordCounter().get("War"), 1);
	}
